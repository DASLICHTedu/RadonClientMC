...
exports.VersionManifestService = void 0;
...
class VersionManifestService {
    ...
    constructor(cacheDir) {
        ...
        this.manifestCacheFile = path_1.default.join(this.cacheDir, 'version_manifest_v2.json');
    }
    /**
    ...
    async getManifest(forceRefresh = false) {
        ...
        try {
            const response = await axios_1.default.get(PISTON_MANIFEST_URL, { timeout: 10000 });
            this.manifestData = response.data;
            fs_1.default.writeFileSync(this.manifestCacheFile, JSON.stringify(this.manifestData, null, 2));
            return this.manifestData;
        }
        catch (err) {
            if (fs_1.default.existsSync(this.manifestCacheFile)) {
                try {
                    this.manifestData = JSON.parse(fs_1.default.readFileSync(this.manifestCacheFile, 'utf8'));
                    return this.manifestData;
                }
                catch {
                    // ignore
            ...
            throw new Error(`Failed to fetch Minecraft version manifest: ${err.message}`);
        }
    }
    /**
     * Fetches specific version details (e.g. 1.8.9.json or 1.21.1.json)
     */
    async getVersionDetail(versionId, gameDir) {
        const localVersionFile = path_1.default.join(gameDir, 'versions', versionId, `${versionId}.json`);
        // If already downloaded and exists locally, return local copy
        if (fs_1.default.existsSync(localVersionFile)) {
            try {
                const localDetail = JSON.parse(fs_1.default.readFileSync(localVersionFile, 'utf8'));
        ...
        fs_1.default.writeFileSync(localVersionFile, JSON.stringify(detail, null, 2));
        return detail;
    }
    /**
     ...
     */
    static getLibraryDownloadInfo(library) {
...
exports.VersionManifestService = VersionManifestService;
