...
import { VersionDetail, ModloaderType, Library, ForgeInstallerProfile, ForgeLibrary, NeoForgeVersion, NeoForgeVersionFile } from '../types';

export class ModloaderService {
  private static FABRIC_META_URL = 'https://meta.fabricmc.net/v2/versions/loader';
  private static QUILT_META_URL = 'https://meta.quiltmc.org/v3/versions/loader';
  private static FORGE_MAVEN_URL = 'https://files.minecraftforge.net/net/minecraftforge/forge';
  private static FORGE_INSTALLER_URL = 'https://maven.minecraftforge.net/net/minecraftforge/forge';
  private static NEOFORGE_MAVEN_URL = 'https://maven.neoforged.net/releases/net/neoforged';

  /**
   * Resolves and merges a modloader (Fabric/Quilt/Forge/NeoForge) into the base Minecraft VersionDetail
   */
  public static async resolveModloaderVersion(
    baseVersion: VersionDetail,
   ...
   */
  private static async resolveFabric(baseVersion: VersionDetail, gameDir: string): Promise<VersionDetail> {
    const gameVersion = baseVersion.id;

    try {
      // 1. Get latest compatible loader version
      const loadersRes = await axios.get<Array<{ loader: { version: string; stable: boolean } }>>(
        `${ModloaderService.FABRIC_META_URL}/${gameVersion}`,
        { timeout: 8000 }
      );
      ...
      const profileRes = await axios.get(profileUrl, { timeout: 8000 });
      const fabricProfile = profileRes.data;

      // 3. Merge libraries and mainClass
      const mergedLibraries: Library[] = [...baseVersion.libraries];
      ...
      const mergedDetail: VersionDetail = {
        ...baseVersion,
        id: fabricVersionId,
      ...
      fs.writeFileSync(
        ...
        JSON.stringify(mergedDetail, null, 2)
      );
   ...
   */
  private static async resolveQuilt(baseVersion: VersionDetail, gameDir: string): Promise<VersionDetail> {
    const gameVersion = baseVersion.id;

    try {
      const loadersRes = await axios.get<Array<{ loader: { version: string } }>>(
        `${ModloaderService.QUILT_META_URL}/${gameVersion}`,
        { timeout: 8000 }
      );
      ...
      const mergedLibraries: Library[] = [...baseVersion.libraries, ...(quiltProfile.libraries || [])];

      const quiltVersionId = `quilt-loader-${latestLoader}-${gameVersion}`;
      const quiltVersionDir = path.join(gameDir, 'versions', quiltVersionId);
      if (!fs.existsSync(quiltVersionDir)) {
   ...
   */
  private static async resolveForge(baseVersion: VersionDetail, gameDir: string): Promise<VersionDetail> {
    const gameVersion = baseVersion.id;
    ...
    try {
      ...
      const forgeVersionDir = path.join(gameDir, 'versions', forgeVersionId);
      ...
      const versionJsonPath = path.join(forgeVersionDir, `${forgeVersionId}.json`);
      if (fs.existsSync(versionJsonPath)) {
        try {
          const existingDetail = JSON.parse(fs.readFileSync(versionJsonPath, 'utf8'));
      ...
      try {
        // Download installer
        const installerResponse = await axios.get(installerUrl, {
        ...
        // Save installer
        const installerPath = path.join(forgeVersionDir, installerFilename);
        fs.writeFileSync(installerPath, Buffer.from(installerResponse.data));
        ...
        // Save version JSON
        fs.writeFileSync(versionJsonPath, JSON.stringify(mergedDetail, null, 2));
   ...
   */
  private static async extractForgeProfile(installerPath: string): Promise<ForgeInstallerProfile | null> {
   ...
   */
  private static extractForgeArguments(jsonStr: string): string[] {
    try {
   ...
   * Evaluate Forge rule
   */
  private static evaluateRule(rules: any): boolean {
    // Simplified rule evaluation for Forge
    if (!rules || !rules.os) return true;
    
    const os = rules.os;
    if (os.name && os.name !== 'windows') return false;
    if (os.arch && os.arch !== 'x86_64') return false;
   ...
   */
  public static evaluateRules(rules: any): boolean {
   ...
   */
  private static convertForgeLibraries(forgeLibs: ForgeLibrary[]): Library[] {
    const result: Library[] = [];
    
    for (const lib of forgeLibs) {
      if (!lib.clientreq && lib.serverreq) continue; // Skip server-only
   ...
   */
  public static async getForgeVersions(mcVersion: string): Promise<Array<{ version: string; stable: boolean }>> {
    try {
      ...
      const versions: Array<{ version: string; stable: boolean }> = [];
      ...
      if (versions.length === 0) {
        const knownVersions: Record<string, Array<{ version: string; stable: boolean }>> = {
   ...
   */
  private static async resolveNeoForge(baseVersion: VersionDetail, gameDir: string): Promise<VersionDetail> {
    const gameVersion = baseVersion.id;
    ...
    try {
      ...
      const neoVersionDir = path.join(gameDir, 'versions', neoVersionId);

      if (!fs.existsSync(neoVersionDir)) {
        fs.mkdirSync(neoVersionDir, { recursive: true });
      }

      // Check if version JSON already exists
      const versionJsonPath = path.join(neoVersionDir, `${neoVersionId}.json`);
      if (fs.existsSync(versionJsonPath)) {
        try {
          const existingDetail = JSON.parse(fs.readFileSync(versionJsonPath, 'utf8'));
      ...
      // Download installer
      try {
        const installerResponse = await axios.get(installerFile.url, {
          responseType: 'arraybuffer',
          timeout: 60000,
          headers: { 'User-Agent': 'RadonClient/1.0.0' }
        });

        const installerPath = path.join(neoVersionDir, installerFile.path);
        fs.writeFileSync(installerPath, Buffer.from(installerResponse.data));
        ...
        let neoLibraries: Library[] = [];
        let neoArguments = { jvm: [] as string[], game: [] as string[] };
   ...
   */
  public static async getNeoForgeVersions(mcVersion: string): Promise<NeoForgeVersion[]> {
    try {
      ...
      const versions: string[] = versionMatches.map((m: string) => m.replace(/<\/version>$/, '').replace(/^<version>/, ''));
      
      // Filter versions for this Minecraft version
      const neoVersions: NeoForgeVersion[] = [];
      ...
      const knownNeoVersions: Record<string, NeoForgeVersion[]> = {
