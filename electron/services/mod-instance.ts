...
import { ModInstance, ModInstanceMod, ModInstanceSettings, ModloaderType, ModSourceType, ModrinthVersion, CurseForgeFile, ModrinthSearchResult, CurseForgeSearchResult } from '../types';
import { ModManagerService } from './mod-manager';
import { VersionManifestService } from './version-manifest';

export class ModInstanceService {
  private configDir: string;
  private instancesFile: string;
  private instances: ModInstance[] = [];
  private activeInstanceId: string | null = null;

  constructor(configDir: string) {
    this.configDir = configDir;
    this.instancesFile = path.join(this.configDir, 'mod-instances.json');
    this.loadInstances();
  }
  ...
  private loadInstances(): void {
    if (fs.existsSync(this.instancesFile)) {
      try {
        const data = JSON.parse(fs.readFileSync(this.instancesFile, 'utf8'));
  ...
  private saveInstances(): void {
    try {
      ...
      fs.writeFileSync(
        this.instancesFile,
        JSON.stringify({ instances: this.instances, activeInstanceId: this.activeInstanceId }, null, 2)
      );
    } catch (err) {
   ...
   */
  public getInstances(): ModInstance[] {
   ...
   */
  public getActiveInstance(): ModInstance | null {
   ...
   */
  public getInstanceById(id: string): ModInstance | null {
   ...
   */
  public updateInstance(id: string, updates: Partial<ModInstance>): ModInstance | null {
   ...
   */
  public setActiveInstance(id: string): boolean {
   ...
   */
  public addModToInstance(
    instanceId: string,
    mod: Omit<ModInstanceMod, 'id'>
  ): ModInstanceMod | null {
   ...
   */
  public removeModFromInstance(instanceId: string, modId: string): boolean {
    const instance = this.instances.find(i => i.id === instanceId);
    if (!instance) {
      return false;
    }
   ...
   */
  public toggleModInInstance(instanceId: string, modId: string): boolean {
    const instance = this.instances.find(i => i.id === instanceId);
    if (!instance) {
      return false;
    }
   ...
   */
  public async installInstanceMods(
    instanceId: string,
    gameDir: string
    ...
    for (let i = 0; i < modsToInstall.length; i++) {
      ...
      try {
        const result = await ModManagerService.installMod(
          mod.projectId,
          instance.modloader,
   ...
   */
  public async updateInstanceMod(
    instanceId: string,
    modId: string,
    versionId: string
  ): Promise<{ success: boolean; error?: string; versionName?: string }> {
    const instance = this.instances.find(i => i.id === instanceId);
    if (!instance) {
    ...
    try {
      ...
      const instancePath = path.join(gameDir, 'instances', instanceId);
      
      const result = await ModManagerService.installSpecificModVersion(
        mod.projectId,
        versionId,
        instancePath
      );
   ...
   */
  public static async getAvailableModloaderVersions(
    modloader: ModloaderType,
    mcVersion: string
  ): Promise<string[]> {
    // This is a simplified version - in a full implementation, you'd fetch from each modloader's API
    const mockVersions: Record<ModloaderType, Record<string, string[]>> = {
      fabric: {
   ...
   */
  public static async searchModsForInstance(
    query: string,
    modloader: ModloaderType,
    mcVersion: string,
    ...
    if (source === 'modrinth') {
      const modrinthResults = await ModManagerService.searchModrinth(
        query,
        loaderParam,
        mcVersion
      );
    ...
    if (source === 'curseforge') {
      const curseforgeResults = await ModManagerService.searchCurseForge(query, mcVersion, loaderParam);
    ...
    const [modrinthResults, curseforgeResults] = await Promise.all([
      ModManagerService.searchModrinth(query, loaderParam, mcVersion),
      ModManagerService.searchCurseForge(query, mcVersion, loaderParam)
    ]);
